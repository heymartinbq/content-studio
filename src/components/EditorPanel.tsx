  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      >
        ...
        {active && (
          <div className={`absolute -left-1 w-1 h-6 rounded-r-full transition-all duration-300 ${color.replace("text-", "bg-")}`} />
        )}
      </button>

      {/* Tooltip */}
      {isHovered && !disabled && (
        <div className="absolute left-full ml-4 px-3 py-1.5 bg-neutral-900 border border-white/10 rounded-lg text-[10px] font-black uppercase tracking-widest text-white whitespace-nowrap z-50 shadow-2xl pointer-events-none animate-in fade-in slide-in-from-left-2 duration-200">
          {label}
...
export default function EditorPanel() {
  ...
  return (
    <div
      ...
      className="absolute top-8 right-8 bottom-8 w-[460px] bg-neutral-950/90 backdrop-blur-3xl border border-white/10 rounded-[3.5rem] text-white shadow-[0_64px_128px_-32px_rgba(0,0,0,1)] z-40 overflow-hidden flex animate-in fade-in slide-in-from-right-8 duration-500"
    >
      ...
      <div id="sidebar-nav-rail" className="w-[80px] flex flex-col items-center py-10 bg-white/[0.02] border-r border-white/5 shrink-0">
        ...
        <div className="flex flex-col gap-4 pb-4">
          <NavButton id="undo" icon={Undo2} label="Deshacer (Ctrl+Z)" active={false} color="text-white" onClick={actions.undo} />
          <NavButton id="redo" icon={Redo2} label="Rehacer (Ctrl+Shift+Z)" active={false} color="text-white" onClick={actions.redo} />
        </div>
      </div>

      {/* Main Content Area */}
      <div id="panel-content-area" className="flex-1 p-10 overflow-y-auto custom-scrollbar-none">
        <div key={effectiveTab} className="animate-in fade-in slide-in-from-bottom-2 duration-300">
          {/* Header */}
            ...
            <div id="tab-content-media" className="space-y-10">
              ...
              <div id="section-webcam-config" className="space-y-6 pt-10 border-t border-white/5">
                <div className="flex items-center justify-between">
                  ...
                  >
                    <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow-lg transition-transform duration-300 ${config.useWebcam ? "translate-x-6" : "translate-x-0"}`} />
                  </button>
                </div>
              </div>
            </div>
